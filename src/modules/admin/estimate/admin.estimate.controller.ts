...
@Controller('admin/estimate')
export class AdminEstimateController {
  constructor(private readonly service: AdminEstimateService) {}
  ...
  async excelTest(@Res() res: Response) {
    try {
      ...
      // 7. 고정된 위치에 데이터 삽입
      worksheet.getCell('F4').value = estimateData.issueDate; // 발신일
      worksheet.getCell('F7').value = estimateData.recipient; // 수신
      worksheet.getCell('F9').value = estimateData.personInCharge; // 담당자
      worksheet.getCell('E10').value = estimateData.email; // 이메일
      worksheet.getCell('E11').value = estimateData.contact; // 연락처
